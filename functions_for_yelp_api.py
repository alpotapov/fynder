...
def calculate_category_grade(category_notes, user_category_choice):
    ...
    sumi = binary_Rest * category_notes[9] + binary_Culture * category_notes[6] + binary_Leisure * category_notes[
        7] + binary_Shopping * category_notes[8]
    return sumi
...
def calculate_time_grade(category_notes, user_time_choice):
    ...
    binary_Evening = 0
    binary_Morning = 0
    if "Noon" in user_time_choice:
        binary_Noon = 1
    if "Evening" in user_time_choice:
        binary_Evening = 1
    if "Morning" in user_time_choice:
        binary_Morning = 1
...
def calculate_final_grade(category_notes, nMan, nWoman, kids_count, goodForKids, user_category_choice,
                          user_time_choice):
    "Returns double"
    a1 = category_notes
    groupGrade1 = group_grade(nWoman, nMan)
    return calculate_category_grade(a1, user_category_choice) * calculate_manwoman_grade(groupGrade1, a1, kids_count,
                                                                                         goodForKids) * calculate_time_grade(
        a1, user_time_choice)
